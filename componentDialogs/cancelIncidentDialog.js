...
class cancelIncidentDialog extends ComponentDialog {
    constructor(conversationState,userState) {
        super('cancelIncidentDialog');
...
await step.context.sendActivity({
    text: 'Enter incident details for cancellation:',
...
async confirmStep(step){
    ...
    return await step.prompt(CONFIRM_PROMPT, 'Are you sure that all values are correct and you want to CANCEL the incident?', ['yes', 'no']);
}
async summaryStep(step){
    if(step.result===true)
    {
      // Business 
      await step.context.sendActivity("Incident successfully cancelled. Your incident id is : 12345678")
      endDialog = true;
